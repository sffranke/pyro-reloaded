...
class SpotMicro:
    ...
    def update_lines(self, coords):
        if self.mode == "angles": # or self.mode == "calib":
            ...
            #self.print_angles(deg_arr)
            data_to_send = json.dumps(deg_arr).encode()  # Serialize the tuple to a JSON string and then to bytes
            print ("data_to_send: ",data_to_send)
            # TCP/IP client setup
            host = 'mini.local'  # Or use the Raspberry Pi's IP address
            port = 65432  # The port should match the one used by your server on the Raspberry Pi
